...
std::string output = "";
std::string idF = "", idFO = "", idFL = "", idT="", idTO="";
bool isright = false;
int level = 0;
    ...
    switch (input_token) {
        ...
        case t_lparen:
			...
			if(isright){
				//if (level == 0)
					//output += idFO;
				//cout << "Added to output. OUTPUT is currently "<<output<<endl;
				//idF = "";
			}
			else idF = idFO;
			if(level == 0){
...
void mul_op () {
    ...
    switch (input_token) {
        case t_mul:
			...
			cout << "At mul_op, idF is currently "<<idF<<endl;
			idF = ("(* "+idF+" ");
            match (t_mul);
            break;
        case t_div:
