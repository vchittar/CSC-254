...
| AST_check of ast_e
and ast_e =
| AST_binop of (string * ast_e * ast_e) (*binary operators*)
| AST_id of string
| AST_num of string;;

let rec ast_ize_P (p:parse_tree) : ast_sl =
  (*p should go to SL then end of file*)
  match p with
  | PT_nt ("P",[sl; PT_term "$$"])
		-> ast_ize_SL sl
...
and ast_ize_SL (sl:parse_tree) : ast_sl =
  (*SL should go to S and then another SL if applicable or just epsilon*)
  match sl with
  | PT_nt ("SL", []) -> []
  | PT_nt ("SL", [s; slnew])
		-> (ast_ize_S s) :: (ast_ize_SL slnew)
  | _ -> raise (Failure "malformed parse tree in ast_ize_SL")

and ast_ize_S (s:parse_tree) : ast_s =
  (*S go to assign, read, write, if, do and check*)
  match s with
...
and ast_ize_expr (e:parse_tree) : ast_e =
  ...
  (*Variable names: er = E for case R, etr = ET for case R. Same with following*)
  match e with
  | PT_nt ("R", [er; etr])
		-> ast_ize_reln_tail (ast_ize_expr er) etr 
  | PT_nt ("E", [te; tte])
		-> ast_ize_expr_tail (ast_ize_expr te) tte 
  | PT_nt ("T", [ft; ftt])
		-> ast_ize_expr_tail (ast_ize_expr ft) ftt 
  | PT_nt ("F", [PT_id lhs])
		-> AST_id lhs
  | PT_nt ("F", [PT_num num])
		-> AST_num num
  | PT_nt ("F", [PT_term "("; ef; PT_term ")"])
		-> ast_ize_expr ef
...
and ast_ize_expr_tail (lhs:ast_e) (tail:parse_tree) : ast_e =
  ...
  | PT_nt ("FT", [])
      -> lhs
  | PT_nt ("TT", _)
    -> raise (Failure "TT doesn't work")
  | PT_nt ("FT", _)
    -> raise (Failure "FT doesn't work")
...
and interpret_sl (sl:ast_sl) (mem:memory)
  ...
  (*If empty then it's finished. 
  if the state returned from s is Bad or Done (Done shouldn't happen) then it should be error.*)
  match sl with
  | [] -> (Good, mem, inp, outp)
  | h::t -> let (stat,memo,inpu,outpu) = interpret_s h mem inp outp in
    match stat with
	| Good -> interpret_sl t memo inpu outpu
	| _ -> (Bad, mem, inp, outp)
...
(*Assign new memory for values given*)
(*TODO: change existing value of a variable*)
and interpret_assign (lhs:string) (rhs:ast_e) (mem:memory)
                     (inp:string list) (outp:string list)
    : status * memory * string list * string list =
  let (returnvalue, _) = interpret_expr rhs mem in
	match returnvalue with
	| Value i -> let rec add_or_change var value memo memoorig= 
	             (match memo with
				 (*Check if the variable is already initialized. If so, change that value. Else, add this new variable to memory*)
				 | h::t
				   -> let (vars, value_var) = h in
                      if vars = var then (memoorig@[(var,value)])@t else add_or_change var value t (memoorig@[h])
		         | [] -> memoorig@[(var, value)])
				 in
				 let memnew = add_or_change lhs i mem [] in (Good, memnew, inp, outp)
	| Error i -> (Bad, mem, inp, outp)
	

and interpret_read (id:string) (mem:memory)
                   (inp:string list) (outp:string list)
    : status * memory * string list * string list =
  (*Errors are printed right when it is detected. Same with following*)
  match inp with
  | [] -> print_string "unexpected end of input";
          (Bad, mem, inp, outp)
  | h::t -> try (Good, mem@[(id, (int_of_string h))], t, outp) with Failure "int_of_string" -> 
...
and interpret_write (expr:ast_e) (mem:memory)
                    (inp:string list) (outp:string list)
    : status * memory * string list * string list =
  let (returnvalue, _) = interpret_expr expr mem in
  match returnvalue with
  | Value i
    -> print_int i;
	   print_string " ";
...
and interpret_if (cond:ast_e) (sl:ast_sl) (mem:memory)
                 (inp:string list) (outp:string list)
    : status * memory * string list * string list =
  let (stat, _, _, _) = interpret_check cond mem inp outp in
  match stat with
  (*Deal with different results from relation*)
  | Good -> interpret_sl sl mem inp outp
...
and interpret_do (sl:ast_sl) (mem:memory)
  ...
  | h::t 
    -> (match h with
	  (*Check the condition. If meet, execute the expression and recurse. If false, end, but the program is still normal. If Bad, the program has some problem*)
	  | AST_check cond -> let (boolean,_,_,_) = interpret_check cond mem inp outp in
	                      (match boolean with 
						  | Good -> let (stat, memnew, inpnew, outpnew) = interpret_sl t mem inp outp in
	                                (match stat with
									 |Good -> interpret_do sl memnew inpnew outpnew
									 |_ -> (Bad, mem, inp, outp))
						  | Done -> (Good, mem, inp, outp)
						  | Bad -> (Bad, mem, inp, outp))
	  | _ -> (Bad, mem, inp, outp))
...
and interpret_check (cond:ast_e) (mem:memory)
                    (inp:string list) (outp:string list)
    : status * memory * string list * string list =
  (*Check conditional statements here*)
  let interpret_check_helper oper left right = 
    match left with
	| (Error i, _) -> Bad
	| (Value i, _) ->
	  match right with
	  ...
	  | (Value j, _) -> 
	    match oper with
		| "==" -> if i = j then Good else Done
		| "<>" -> if i = j then Done else Good
		| ">" -> if i > j then Good else Done
		| "<" -> if i < j then Good else Done
		| ">=" -> if i >= j then Good else Done
		| "<=" -> if i <= j then Good else Done
		...
		in
  match cond with
  | AST_binop (op, lhs, rhs) -> (interpret_check_helper op (interpret_expr lhs mem) (interpret_expr rhs mem), mem, inp, outp)
  | _ -> (Bad, mem, inp, outp)

and interpret_expr (expr:ast_e) (mem:memory) : value * memory =
  (*algorithm: separate expression to left hand side and right hand side. 
    Keep recursing each hand side until they are either id or number, and then do calculation*)
  let interpret_expr_helper oper left right = 
    match left with
	| (Error i, _) -> Error "Value cannot be evaluated"
	| (Value i, _) ->
	  match right with
	  | (Error j, _) -> Error "Value cannot be evaluated"
	  | (Value j, _) -> 
	    match oper with
		| "+" -> Value (i+j)
		| "-" -> Value (i-j)
		| "*" -> Value (i*j)
		| "/" -> (match j with
		         | 0 -> print_string "divide by 0";
				        Error "divide by 0"
				 | _ -> Value (i/j))
		| _ -> Error "Value cannot be evaluated" 
	in
  match expr with
  | AST_binop (op, lhs, rhs) 
    -> ((interpret_expr_helper op (interpret_expr lhs mem) (interpret_expr rhs mem)), mem)
  | AST_id str -> let rec find_mem var mem= 
		   match mem with
		   | h::t 
		     -> let (vars, value_var) = h in
               if vars = var then Value value_var else find_mem var t
		   | [] -> print_string var;
		           print_string ": symbol not found";
		           Error "Variable not found"
         in
		 ((find_mem str mem), mem)
...
let test_case = "check 3+8*4==2+9";;
let test_case_syntax_tree = ast_ize_P (parse ecg_parse_table test_case);;
