...
and ast_ize_SL (sl:parse_tree) : ast_sl =
  ...
  | PT_nt ("SL", []) -> []
  | PT_nt ("SL", [s; slnew])
		-> (ast_ize_S s) :: (ast_ize_SL slnew) (*List contains s and slnew, also can use append?*)
  (*
     your code here ...
  *)
...
and ast_ize_expr (e:parse_tree) : ast_e =
  ...
  match e with
  | PT_nt ("R", [er; etr])
		-> ast_ize_reln_tail (ast_ize_expr er) etr (*etr or slnew?*)
  | PT_nt ("E", [te; tte])
		-> ast_ize_expr_tail (ast_ize_expr te) tte (*tte or slnew?*)
  | PT_nt ("T", [ft; ftt])
		-> ast_ize_expr_tail (ast_ize_expr ft) ftt (*ftt or slnew?*)
  | PT_nt ("F", [PT_id lhs])
		-> AST_id lhs
  | PT_nt ("F", [PT_num num])
		-> AST_num num
...
and ast_ize_reln_tail (lhs:ast_e) (tail:parse_tree) : ast_e =
  ...
  | PT_nt ("ET", [PT_nt("ro", [PT_term "=="]); rhs])
  		-> AST_binop ("==", lhs, ast_ize_expr rhs) (*If runtime error occurs, check ast_ize_expr lhs vs just lhs*)
  | PT_nt ("ET", [PT_nt("ro", [PT_term "<"]); rhs])
  ...
  | PT_nt ("ET", [PT_nt("ro", [PT_term "<>"]); rhs])
  		-> AST_binop ("<>", lhs, ast_ize_expr rhs)
  (*| PT_nt ("ET", [PT_term "=="; rhs])
  		-> AST_binop ("==", lhs, ast_ize_expr rhs) Alternative*)
  | PT_nt ("ET", [])
  		-> lhs
...
and ast_ize_expr_tail (lhs:ast_e) (tail:parse_tree) : ast_e =
  ...
  | PT_nt ("TT", [PT_nt ("ao", [PT_term ao]); te; tte]) (*rhs here contains both te and tte*)
      (*-> AST_binop (ao, lhs, ast_ize_expr_tail (ast_ize_expr te) tte) *)
...
and interpret_sl (sl:ast_sl) (mem:memory)
  ...
  match sl with
  | [] -> (Good, mem, inp, outp)
  | h::t -> let (_,memo,inpu,outpu) = interpret_s h mem inp outp in
    interpret_sl t memo inpu outpu
	(*This is not checking good/bad/done right now*)
...
and interpret_assign (lhs:string) (rhs:ast_e) (mem:memory)
    ...
    : status * memory * string list * string list =
  (* your code should replace the following line *)
  (*(Good, (mem::lhs), interpret_expr rhs, )*)
  let (returnvalue, _) = interpret_expr rhs mem in
	match returnvalue with
	| Value i -> (Good, mem@[(lhs,i)], inp, outp)
...
and interpret_read (id:string) (mem:memory)
  ...
  match inp with
  | [] -> (Bad, mem, inp, outp)
  | h::t -> (Good, mem@[(id, (int_of_string h))], t, outp)
  (*(Good, mem, inp, outp)*)
...
and interpret_write (expr:ast_e) (mem:memory)
  ...
  | Value i
    -> print_int i;
	   (Good, mem, inp, outp)
  | Error i
    -> print_string "ERROR. Cannot write file";
	 (Bad, mem, inp, outp)
  (* your code should replace the following line *)
  (*(Good, mem, inp, outp)*)
...
and interpret_if (cond:ast_e) (sl:ast_sl) (mem:memory)
  ...
  (*match returnvalue with
  | Value i -> (Good, mem, inp, outp)
  | Error i -> interpret_sl sl mem inp outp 
  (Good, mem, inp, outp)*)
...
and interpret_check (cond:ast_e) (mem:memory)
  ...
  | _ -> (Bad, mem, inp, outp)
  (*let cond op lhs rhs: value = 
  match (op, (lhs), (rhs) ) with
  | Good -> ("<", Value x, Valye y) 
  (Done, mem, inp, outp)*)
