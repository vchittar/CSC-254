...
and ast_ize_reln_tail (lhs:ast_e) (tail:parse_tree) : ast_e =
  ...
  match tail with
  | PT_nt ("ET", [lhs; PT_nt("ro", [PT_term "=="]); rhs])
  		-> AST_binop ("==", ast_ize_expr lhs, ast_ize_expr rhs) (*If runtime error occurs, check ast_ize_expr lhs vs just lhs*)
  | PT_nt ("ET", [lhs; PT_nt("ro", [PT_term "<"]); rhs])
  		-> AST_binop ("<", ast_ize_expr lhs, ast_ize_expr rhs)
  | PT_nt ("ET", [lhs; PT_nt("ro", [PT_term ">"]); rhs])
  		-> AST_binop (">", ast_ize_expr lhs, ast_ize_expr rhs)
  | PT_nt ("ET", [lhs; PT_nt("ro", [PT_term "<="]); rhs])
  		-> AST_binop ("<=", ast_ize_expr lhs, ast_ize_expr rhs)
