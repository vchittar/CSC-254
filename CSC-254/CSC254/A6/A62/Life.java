...
// generations.
//
class Worker<Void> implements Callable<Void> {
...
class LifeBoard extends JPanel {
    ...
    public void doGeneration(Integer[] task) throws Coordinator.KilledException {
		...
		//Board is not updated here. 
		//Instead, it is updated in update() method and is called after Executor finishes invokeAll()
        //T = B;  B = A;  A = T;
        if (headless) {
            if (generation % 10 == 0) {
    ...
    public LifeBoard(int N, Coordinator C, UI U,
                     boolean hdless, boolean glider, String filename) {
        n = N;
        c = C;
        u = U;
        headless = hdless;
...
class UI extends JPanel {
    ...
    public UI(int N, RootPaneContainer pane, int pauseIterations,
        ...
        lb = new LifeBoard(N, c, u, headless, glider, filename);
		...
		final JButton stepButton = new JButton("Step"); //Added extra button to step through a single generation
        ...
        runButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
				//Running means not run in steps
				lb.setStep(false);
                if (state == stopped) {
					c.stop();
                    state = running;
    ...
    public void onRunClick() {
      /*Thread w = new Thread(new Worker(lb, c, this));
      w.start();*/
      //Assign rows to each Thread
	  ArrayList<Worker<Void>> workerlist = new ArrayList<Worker<Void>>();
	  for(int i = 0; i < numThreads; i++){
			...
			workerlist.add(new Worker<Void>(lb,c,this,task.toArray(new Integer[0])));
		}
	  //Start a new Thread to run Executor. Otherwise the UI will not be responsive
	  Thread workmanager = new Thread(){public void run(){
